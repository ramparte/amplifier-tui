...
from ..platform_info import amplifier_home, amplifier_projects_dir
from ..constants import (
    SLASH_COMMANDS,
)

# NOTE: ChatInput is imported lazily at each usage site to avoid
# pulling in Textual at import time (keeps core/ Textual-free).
    ...
    def _cmd_snippet(self, text: str) -> None:
        ...
        if subcmd == "clear":
            ...
            if subcmd in self._snippets:
                try:
                    from amplifier_tui.widgets import ChatInput as _ChatInput

                    inp = self.query_one("#chat-input", _ChatInput)
                    inp.clear()
    ...
    def _cmd_draft(self, text: str) -> None:
        ...
        if arg == "load":
            ...
            if draft:
                ...
                input_widget = self.query_one("#chat-input", _ChatInput)
                input_widget.clear()
        ...
        if not lines:
            ...
            try:
                ...
                input_text = self.query_one("#chat-input", _ChatInput).text.strip()
            except Exception:
